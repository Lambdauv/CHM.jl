...
#ifdef PPC_BSTR
/* RWE 6/12/2003 */
EXPORT_DLL struct chmFile* chm_open(BSTR filename);
#else
EXPORT_DLL struct chmFile* chm_open(const char *filename);
#endif

/* close an ITS archive */
EXPORT_DLL void chm_close(struct chmFile *h);

/* methods for ssetting tuning parameters for particular file */
#define CHM_PARAM_MAX_BLOCKS_CACHED 0
EXPORT_DLL void chm_set_param(struct chmFile *h,
                   int paramType,
                   int paramVal);
...
EXPORT_DLL int chm_resolve_object(struct chmFile *h,
                       const char *objPath,
                       struct chmUnitInfo *ui);

/* retrieve part of an object from the archive */
EXPORT_DLL LONGINT64 chm_retrieve_object(struct chmFile *h,
                              struct chmUnitInfo *ui,
                              unsigned char *buf,
                              LONGUINT64 addr,
                              LONGINT64 len);
...
EXPORT_DLL int chm_enumerate(struct chmFile *h,
                  int what,
                  CHM_ENUMERATOR e,
                  void *context);

EXPORT_DLL int chm_enumerate_dir(struct chmFile *h,
                      const char *prefix,
                      int what,
                      CHM_ENUMERATOR e,
                      void *context);
